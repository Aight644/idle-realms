...
// Item tier colors
const TIER_COLORS = {
  1: {color:"#8899aa", label:"Common"},
  2: {color:"#00d4ff", label:"Uncommon"},
  3: {color:"#7b61ff", label:"Rare"},
  4: {color:"#ff006e", label:"Epic"},
  5: {color:"#ffd60a", label:"Legendary"},
};
const getItemTier = (iid) => {
  const it = ITEMS[iid]; if(!it||!it.st) return 1;
  const power = Object.entries(it.st).reduce((s,[k,v]) => s + (typeof v==="number" && Math.abs(v)<1 ? v*100 : v), 0);
  if(power >= 80) return 5;
  if(power >= 45) return 4;
  if(power >= 25) return 3;
  if(power >= 12) return 2;
  return 1;
};
const tierOf = (iid) => TIER_COLORS[getItemTier(iid)] || TIER_COLORS[1];
...
function GameUI({account,onLogout}){
  ...
  useEffect(()=>{
    ...
    const dur=Math.max(1500,(act.t*1000)/speedMult);
    let start=Date.now();
    let rafId;
  ...
  const doEnh=useCallback((sid)=>{
    ...
    if(Math.random()*100<tier.rate){
      ...
      setClog(p=>[...p.slice(-20),"✨ "+ITEMS[iid].n+" upgraded to +"+(cl+1)+"!"+(ms?" 🏅 "+ms.label+"!":"")]);
      setEnhAnim({slot:sid,result:"success",ts:Date.now()});
    }else{
      if(useScroll){
        const newLv=Math.max(0,cl-1);
        setEnh(p=>({...p,[iid]:newLv}));
        setClog(p=>[...p.slice(-20),"📜 Protection Scroll softened the blow! "+ITEMS[iid].n+" dropped to +"+newLv]);
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflow:"auto",padding:"24px 28px"}}>
              ...
              <div style={{}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  {ESLOTS.map(slot=>{const iid=eq[slot.id];const it=iid?ITEMS[iid]:null;const el=iid?(enh[iid]||0):0;
                    ...
                    const borderColor=setData?setData.color+"50":it?tier.color+"50":C.border;
                    const glowColor=it?(setData?setData.color:tier.color):null;
                    return(
                    <div key={slot.id} onMouseEnter={iid?e=>showTip(e,iid):null} onMouseLeave={hideTip}
                      style={{padding:"12px 14px",borderRadius:8,background:C.card,border:"1px solid "+borderColor,boxShadow:glowColor?"0 0 10px "+glowColor+"35":"none",cursor:it?"pointer":"default",transition:"all 0.15s"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                        <div style={{display:"flex",alignItems:"center",gap:6}}>
                          <span style={{fontSize:10,color:C.ts,fontFamily:FONT_BODY}}>{slot.i} {slot.n}</span>
                          {it&&tier&&<span style={{fontSize:8,color:tier.color,fontWeight:700,fontFamily:FONT,letterSpacing:0.5,padding:"1px 6px",borderRadius:6,background:tier.color+"15",border:"1px solid "+tier.color+"30"}}>{tier.label.toUpperCase()}</span>}
                        </div>
                      ...
                      {it?(
                        <div>
                          <div style={{fontSize:12,fontWeight:700,color:setData?setData.color:tier.color,fontFamily:FONT}}>{it.i} {it.n}{el>0?" +"+el:""}</div>
            ...
            {page==="enhancing"&&(()=>{
              ...
              const scrollOn=selSlot?enhUseScroll[selSlot.slot.id]:false;
              ...
              <div style={{maxWidth:600}}>
                ...
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{fontSize:10,color:"#f87171",fontFamily:FONT_BODY}}>⚠ Failed upgrades reset to +0. Use 📜 Protection Scrolls to only lose 1 level instead.</div>
                </div>
                  ...
                  <div style={{display:"flex",gap:16}}>
                    ...
                    {selIt&&(
                      <div style={{flex:1}}>
                          ...
                          {selIt.st&&(
                            <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:8}}>
                              {Object.entries(selIt.st||{}).map(([k,v])=>{
                                const base=v;const boosted=typeof v==="number"?Math.floor(v*(1+enhStatMult(selCl))):v;
                                return(
                        ...
                        {/* Upgrade info + button */}
                        <div style={{padding:"14px 16px",borderRadius:10,background:C.card,border:"1px solid "+(anim?(anim.result==="success"?C.ok:anim.result==="protected"?C.warn:C.bad):C.border),marginBottom:12,boxShadow:anim?(anim.result==="success"?GLOW_OK:anim.result==="protected"?"0 0 12px #ffb70055":GLOW_BAD):"none",transition:"all 0.3s"}}>
                          {anim&&<div style={{fontSize:11,fontWeight:700,color:anim.result==="success"?C.ok:anim.result==="protected"?C.warn:C.bad,fontFamily:FONT,textAlign:"center",marginBottom:8,animation:"pulse 0.6s ease-in-out",letterSpacing:1}}>
                            ...
                            <React.Fragment>
                              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
                                <div style={{textAlign:"center"}}>
                                  <div style={{fontSize:9,color:C.td,fontFamily:FONT,letterSpacing:1,marginBottom:3}}>GOLD</div>
                                  <div style={{fontSize:14,fontWeight:700,color:canAffordGold?C.gold:"#f87171",fontFamily:FONT}}>◈{fmt(selTier.gold)}</div>
                                </div>
                                <div style={{textAlign:"center"}}>
                                  <div style={{fontSize:9,color:C.td,fontFamily:FONT,letterSpacing:1,marginBottom:3}}>MATERIAL</div>
                                  {selTier.mat&&(()=>{const mit=ITEMS[selTier.mat.id];return(
                                    <div style={{fontSize:12,fontWeight:700,color:canAffordMat?C.ok:"#f87171",fontFamily:FONT}}>{mit?.i} ×{selTier.mat.q}</div>
                                  );})()}
                                </div>
                                <div style={{textAlign:"center"}}>
                                  <div style={{fontSize:9,color:C.td,fontFamily:FONT,letterSpacing:1,marginBottom:3}}>SUCCESS</div>
                                  <div style={{fontSize:14,fontWeight:700,color:selTier.rate>=60?C.ok:selTier.rate>=40?C.warn:"#f87171",fontFamily:FONT}}>{selTier.rate}%</div>
                                </div>
                              </div>
                              {/* Milestone + protection row */}
                              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                                {nextMs?<div style={{fontSize:9,color:nextMs[1].color,fontFamily:FONT}}>Next: +{nextMs[0]} {nextMs[1].label} — {nextMs[1].bonus}</div>:<div/>}
                                <div onClick={()=>selSlot&&setEnhUseScroll(p=>({...p,[selSlot.slot.id]:!p[selSlot.slot.id]}))}
                                  style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer",padding:"4px 10px",borderRadius:6,
                                  background:scrollOn&&hasScroll?C.warn+"20":C.bg,border:"1px solid "+(scrollOn&&hasScroll?C.warn+"60":C.border),opacity:hasScroll?1:0.4}}>
                                  <span style={{fontSize:13}}>📜</span>
                                  <span style={{fontSize:9,color:scrollOn&&hasScroll?C.warn:C.td,fontWeight:600,fontFamily:FONT}}>PROTECT ({inv["protection_scroll"]||0})</span>
                                </div>
                              </div>
                              {curMs&&<div style={{fontSize:9,color:curMs[1].color,fontFamily:FONT,marginBottom:8,padding:"3px 8px",borderRadius:6,background:curMs[1].color+"15",border:"1px solid "+curMs[1].color+"30",display:"inline-block"}}>🏅 {curMs[1].label} — {curMs[1].bonus}</div>}
                            </React.Fragment>
                          ):(
                        ...
                        <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
                          {["enhancement_stone","greater_enh_stone","superior_enh_stone","protection_scroll"].map(mid=>{
                            ...
                            return(
                              <div key={mid} style={{padding:"4px 8px",borderRadius:6,background:C.card,border:"1px solid "+C.border,display:"flex",alignItems:"center",gap:4,fontSize:10}}>
                                <span>{it.i}</span>
                                <span style={{color:C.ts,fontFamily:FONT_BODY}}>{it.n}</span>
                                <span style={{color:qty>0?C.acc:C.td,fontWeight:700,fontFamily:FONT}}>{qty}</span>
                              </div>
                            );
                          })}
                        </div>
